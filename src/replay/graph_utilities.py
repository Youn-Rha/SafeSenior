...
class Circle3D:
    def __init__(self, x, y, z, radius=1.0, segments=200):
        self.x = x
        self.y = y
        self.z = z
        self.radius = radius
    ...
    def _create_circle(self):
        ...
        z = self.z + np.zeros_like(x)  # Flat on the X-Y plane
...
class CircleBillboard3D:
    def __init__(self, x, y, z, radius=1.0, view = None):
        self.circle = Circle3D(x, y, z, radius=radius)
        self.view = view
    ...
    def return_circle_mesh(self):
        return self.circle.get_circle_mesh()
...
def color_def(i):
    ...
    if i < 25:
        return colors_qcolor[i]
    else:
        return default_qcolor

# def create_sphere(radius, num_points=100, center=(0, 0, 0)):
...
def getSphereMesh(
    ...
    sphere1 = getSphereTriangles(verts=verts, stacks=stacks, sectors=sectors)
    # sphere2 = getSphereTriangles(verts=rotate_vertices(verts, center, np.radians(90), axis="x"), stacks=stacks, sectors=sectors)
    # sphere3 = getSphereTriangles(verts=rotate_vertices(verts, center, np.radians(180), axis="x"), stacks=stacks, sectors=sectors)
    # sphere4 = getSphereTriangles(verts=rotate_vertices(verts, center, np.radians(270), axis="x"), stacks=stacks, sectors=sectors)
    # sphere5 = getSphereTriangles(verts=rotate_vertices(verts, center, np.radians(90), axis="y"), stacks=stacks, sectors=sectors)
    # sphere6 = getSphereTriangles(verts=rotate_vertices(verts, center, np.radians(270), axis="y"), stacks=stacks, sectors=sectors)
    # all_lines = np.vstack((sphere1, sphere2, sphere3, sphere4, sphere5, sphere6))
...
# 원래 쓰던 3D 모델링 코드드
def getBoxLinesCoordsBoxHead(x, y, z, track_prediction = 1):
    # for debug
...
# 머리 부분을 구형으로 만들기 위해 실험 중인 코드
def getBoxLinesCoordsCircleHead(x, y, z, track_prediction = 1):
    # for debug
    # if track_prediction == 1:
    #     track_prediction = 3
    body_width = 0.4
    ...
    # 파츠를 죄다 합쳐서 리턴   
    all_lines = np.vstack((body_lines, left_arm_lines, right_arm_lines, left_leg_lines, right_leg_lines))
