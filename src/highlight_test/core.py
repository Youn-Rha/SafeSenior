...
class Window(QMainWindow):
    ...
    def displayPointCloudWithCircles(self, frameNum):
        if float(frameNum) < 0:
            return
        self.gl_view.scatter.setVisible(False)
        ...
        toPlot = []
        pointColors = []
        numberOfClustersInFrame = 0
        clustersInFrame = set()
        xPosList = [[] for _ in range(25)]
        yPosList = [[] for _ in range(25)]
        zPosList = [[] for _ in range(25)]
        if 'global_cluster' in self.outputList[0]:
            glob_clust_flag = True
        else:
            glob_clust_flag = False
        for dict in self.outputList:
            if dict['frameNum'] == str(frameNum):
                ...
                if glob_clust_flag:
                    if 0 <= int(float(dict['global_cluster'])):
                        if int(float(dict['global_cluster'])) not in clustersInFrame:
                            gNum = int(float(dict['global_cluster'])) % len(self.colors_qcolor)
                            clustersInFrame.add(gNum)
                            numberOfClustersInFrame += 1
                        pointColors.append(self.colors_qcolor[int(float(dict['global_cluster'])) % len(self.colors_qcolor)])
                        xPosList[gNum].append(float(dict['xPos']))
                        yPosList[gNum].append(float(dict['yPos']))
                        zPosList[gNum].append(float(dict['zPos']))
                    else:
                        pointColors.append(self.default_qcolor)
                else:
                    snr = float(dict['SNR']) / 5
        ...
        nppointColors = np.array([pg.glColor(c) for c in pointColors], dtype=np.float32)
        npsize = np.array(size)
        self.create_circle(xPosList, yPosList, zPosList, clustersInFrame, radius=0.3)
        self.gl_view.scatter.setData(pos=nptoPlot, color=nppointColors, size=npsize)
        self.gl_view.scatter.setVisible(True)
    ...
    def create_circle(self, x, y, z, clusterNum, radius=1.0, segments=200):
        theta = np.linspace(0, 2 * np.pi, segments)
        for num in clusterNum:
            pointList = []
            xFirst = np.mean(x[num]) + radius * np.cos(theta)
            yFirst = np.mean(y[num]) + radius * np.sin(theta)
            zFirst = np.mean(z[num]) + np.zeros_like(xFirst)
            firstPoints = np.column_stack((xFirst, yFirst, zFirst))
            xSec = np.mean(x[num]) + radius * np.cos(theta)
            ySec = np.mean(y[num]) + np.zeros_like(xSec)
            zSec = np.mean(z[num]) + radius * np.sin(theta)
            secPoints = np.column_stack((xSec, ySec, zSec))
            yTh = np.mean(y[num]) + radius * np.cos(theta)
            xTh = np.mean(x[num]) + np.zeros_like(yTh)
            zTh = np.mean(z[num]) + radius * np.sin(theta)
            thPoints = np.column_stack((xTh, yTh, zTh))
            pointList.append(firstPoints)
            pointList.append(secPoints)
            pointList.append(thPoints)
            npPointList = np.concatenate(pointList, axis=0)
            colors = [(self.colors_rgba[num][0] / 255, self.colors_rgba[num][1] / 255,
                       self.colors_rgba[num][2] / 255, self.colors_rgba[num][3])] * npPointList.shape[0]
            npColors = np.array(colors)
            self.gl_view.ellipsoids[num].setData(pos=npPointList, color=npColors, width=2, antialias=True, mode='line_strip')
            self.gl_view.ellipsoids[num].setVisible(True)


    def updateFrameInfo(self, frameNum):
    ...
    def updateFrame(self):
        self.updateFrameInfo(self.currentFrame)
        self.displayPointCloudWithCircles(self.csvMetaData[self.currentFrame - 1][CSV_META.FRAME_NUM])
...
if __name__ == '__main__':
    ...
    main.show()
    main.displayPointCloudWithCircles(0 + int(main.firstFrameNum))
    sys.exit(app.exec_())
